...
In dieser Übung wollen wir auf der Kommandozeile Benutzer anlegen und verwalten lernen. Ferner werden wir hinter die Kulissen der Benutzerverwaltung unter Linux blicken, indem wir die Inhalte der Konfigurationsdateien analysieren.

= B~~~ Plattform und DurchführungÜbungsziel
#v(2mm)

Sie benötigen für diese Übung ein Linux-System, auf dem Sie als Benutzer das Recht haben, Superuser-_(root)_-Privilegien zu erhalten (mit `sudo`#footnote[`sudo`: startet einen Prozess mit den Rechten eines anderen Benutzers (meistens `root`). Das geht nur, wenn man (direkt oder als Mitglied einer Gruppe) in der Datei `/etc/sudoers` eingetragen ist!] oder `su`#footnote[`su` (*s* witch *u* ser oder *s* ubstitute *u* ser identity)]). Am besten verwenden Sie eine der vorbereiteten virtuellen Maschinen -- auf den VMs mit GUI ist dafür der User `junioradmin` eingerichtet!

Dokumentieren Sie in Ihrem Protokoll *jede* der verwendeten Befehlszeilen, eventuell wichtige Ausgaben der Befehle und die Antwort auf *alle* Fragen!
...
- Hauptgruppe (= *Login Group*): `avatare`

Beantworten Sie dazu folgende Fragen:

+ Welche Befehlszeile^[Eselsbrücke: *`d`*as *`c`*hamäleon *`m`*ag *`g`*erne *`G`*rüne *`s`*chmetterlinge ;-)] haben Sie zum Anlegen des Benutzers verwendet?
...
#count6[Geben Sie dem Benutzer `max` ein _gutes_ Passwort!
...
#count[Erzeugen Sie am System folgende Benutzer mit Hilfe einer einfachen *Skriptdatei* _(ins Protokoll!)_, mit dem Dateinamen `useranlegen.sh` _(vorher alle 5 Tipps lesen!)_:

_Tipp 1:_ Ihre Skripdatei erstellen Sie am einfachsten in einem Editor (`nano`  / `vi`). Geben Sie für jeden Benutzer einen `useradd`-Befehl pro Zeile an (ohne `sudo`). Sie können die Datei dann
mit `sudo sh useranlegen.sh` ausführen#footnote[In der nächsten Übung lernen Sie, wie man eine Datei ausführbar macht, so dass Sie die Scriptdatei ohne `sh` starten können]!

_Tipp 2:_ Es empfiehlt sich, zuerst die Gruppen
(`buchhaltung`, `mgmt`, …) sowie die übergeordneten Ordner der Benutzerverzeichnisse (`/home/bh`, `/home/mgmt`,…) anzulegen!

_Tipp 3:_ Wir wollen auch die Passwortvergabe über das Skript (Batchdatei) automatisieren! Verwenden Sie dazu den Befehl `chpasswd`(mittels der Befehlszeile: `echo username:password |v  chpasswd` )  -- was ist daran eventuell problematisch?

_Tipp 4:_ Es wäre klug, vor dem Anlegen (einmalig, nicht im Skript) dieser Benutzer und Gruppen Sicherungskopien der drei wesentlichen Dateien der Benutzerverwaltung zu machen -- _welche Dateien sind das und wie macht man das?_

_Tipp 5:_ ganz oben in der Skriptdatei sind oft folgende einfache "Debug-Optionen" hilfreich:

- `set -e`   bricht im Fehlerfall sofort ab (Skript läuft nicht weiter)

- `set -v -x` gibt jeden Befahl vor der Ausführung nochmals aus

- in der Skriptdatei beginnen Kommentare mit `#`
]

Liste der Benutzer:

Achtung: Statt _IhrLogin_ verweden Sie Ihren Schul-Loginnamen mit Ihrer 4-stelligen Nummer und vorangestelltem "s" (z.B. s9999)!
...
#count[Schreiben und testen Sie eine zweite Skriptdatei mit dem Dateinamen `userloeschen.sh`, die alle obigen Benutzer mitsamt ihren Heimatverzeichnissen wieder _löscht_!]

#count[Mit welchem Kommandozeilen-Befehl übeprüfen Sie, welche Benutzer-ID und welche Gruppen _IhrLogin_ zugewiesen wurde? Fügen Sie den Befehl und dessen Ausgabe in Ihr Protokoll ein!]
